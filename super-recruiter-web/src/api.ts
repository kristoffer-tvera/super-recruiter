import { type PlayerResponse, PlayerStatus, type PlayerFilter, type AdminConfig } from "./types";
...
export async function fetchPlayers(filter?: PlayerFilter): Promise<PlayerResponse[]> {
    const params = new URLSearchParams();

    if (filter?.statuses && filter.statuses.length > 0) {
        filter.statuses.forEach((status) => params.append("status", String(status)));
    }
    if (filter?.classes && filter.classes.length > 0) {
        filter.classes.forEach((cls) => params.append("playerClass", cls));
    }
...
export async function fetchPlayerByCharacterAndRealm(realmSlug: string, characterName: string): Promise<PlayerResponse> {
    const res = await fetch(`${BASE}/players/lookup/${encodeURIComponent(realmSlug)}/${encodeURIComponent(characterName)}`, {
        headers: withApiKey(),
    });
    if (!res.ok) throw new Error("Failed to fetch player");
    return res.json();
...
export async function updatePlayerStatus(realmSlug: string, characterName: string, status: PlayerStatus): Promise<PlayerResponse> {
    const res = await fetch(`${BASE}/players/${encodeURIComponent(realmSlug)}/${encodeURIComponent(characterName)}/status`, {
        method: "PUT",
